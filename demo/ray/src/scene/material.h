...
class Material {
  ...
  public:
    ...
    // (i.e., mapped textures)
    Vec3d ke(const isect& i) const {
      return _ke.value(i);
    }
    Vec3d ka(const isect& i) const {
      return _ka.value(i);
    }
    Vec3d ks(const isect& i) const {
      return _ks.value(i);
    }
    Vec3d kd(const isect& i) const {
      return _kd.value(i);
    }
    Vec3d kr(const isect& i) const {
      return _kr.value(i);
    }
    Vec3d kt(const isect& i) const {
      return _kt.value(i);
    }
    ...
    double index(const isect& i) const {
      return _index.intensityValue(i);
    }

    // setting functions accepting primitives (Vec3d and double)
    void setEmissive(const Vec3d& ke)     {
      _ke.setValue(ke);
    }
    void setAmbient(const Vec3d& ka)      {
      _ka.setValue(ka);
    }
    void setSpecular(const Vec3d& ks)     {
      _ks.setValue(ks);
    }
    void setDiffuse(const Vec3d& kd)      {
      _kd.setValue(kd);
    }
    void setReflective(const Vec3d& kr)   {
      _kr.setValue(kr);
    }
    void setTransmissive(const Vec3d& kt) {
      _kt.setValue(kt);
    }
    ...
    void setEmissive(const MaterialParameter& ke)            {
      _ke = ke;
    }
    void setAmbient(const MaterialParameter& ka)             {
      _ka = ka;
    }
    void setSpecular(const MaterialParameter& ks)            {
      _ks = ks;
    }
    void setDiffuse(const MaterialParameter& kd)             {
      _kd = kd;
    }
    ...
    void setIndex(const MaterialParameter& index)            {
      _index = index;
    }
